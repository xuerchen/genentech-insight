...
# A, paths
train=['../input/trainx.csv','../noleak/phy_noleak_group_train.csv']#,'../noleak/dp_encode4.csv']
test=train
trainsvm=[]#['../../boat/claim/full_line_claim_id_train_small.svm']
testsvm=trainsvm
submission = 'predictions/select12cv.csv'  # path of to be outputted submission file
weightmap={}
screenmap={}
countmap={}
feamap={}
...
for e in xrange(epoch):
    ...
    for t, fea, x, y in data(train,trainsvm, D, writefeamap=True):  # data is a generator
        ...
        f.write('%s,%f,%s\n'%(t,y,fea))
        if count%10000==0:
            print count,'important fea',len(feamap)
f.close()
...
#'specialty_code'],row['CBSA'],row['state'
diagx={1:'specialty_code',2:'CBSA',3:'state'}
print 'important features:',len(feamap)
f=open('feamap12.csv','w')
