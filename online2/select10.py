...
# A, paths
train=['../input/trainx.csv','../noleak/dp_encode5.csv','../noleak/dp_encode6.csv']#,'../noleak/dp_encode4.csv']
test=train
trainsvm=[]#['../../boat/claim/full_line_claim_id_train_small.svm']
testsvm=trainsvm
submission = 'predictions/select10cv.csv'  # path of to be outputted submission file
weightmap={}
screenmap={}
countmap={}
feamap={}
...
name='../input/diagnosis_code.csv'
for c,row in enumerate(csv.DictReader(open(name))):
    diag[row['diagnosis_code']]=row['diagnosis_description']

name='../input/surgical_code.csv'
for c,row in enumerate(csv.DictReader(open(name))):
    pro[row['surgical_code']]=row['surgical_description']

print 'important features:',len(feamap)
f=open('feamap10.csv','w')
...
for i in feamap:
    ...
    if des=='':
        if codes in diag:
            ...
            des='s:'+pro[codes]
    f.write('%d,%s,%f,%f,%f,"%s"\n'%(i,' '.join(feamap[i]),weightmap[i],screenmap[i],countmap[i],des))
f.close()
#f=open('score','a')
#f.write('fea %s\n'%(sys.argv[1]))
#f.close()
